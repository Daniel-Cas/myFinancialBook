...
export class LoginService {
  ...
  credentialsValidate(user: string, pass: string) {
    ...
    this.http.get<[]>(`${this._url}enterprise/list`).subscribe((resp) => {
  ...
  registerEnterprise(empresa: string, nit: number) {
    ...
    if (confirmation) {
      ...
      this.http
        .get(
          `${this._url}enterprise/add?nit=${body.nit}&name=${body.name}&direction=${body.direction}&description=${body.description}&phoneNumber=${body.phoneNumber}`
        )
        .toPromise()
        .then((datos: any) => {
